...
//	require_once '../aaplication/config/database.php';
	...
	if ($_GET['get'] == 'paid')
	{
		$q = mysql_query('SELECT * FROM `keys` WHERE timestamp>0');
		$out=array();
	...
	class LTKeys
	{
		...
		{
			$q = mysql_query('SELECT * from `keys` WHERE timestamp = 0 ORDER BY `key`');
			$out = array();
			while ($r = mysql_fetch_assoc($q))
			{
				$out[] = $r['key'];
	...
	{
		$keys = explode("\n", $_POST['keys']);
		mysql_query('DELETE FROM `keys` WHERE timestamp = 0');
		foreach ($keys as $key)
		{
			$key = trim($key);
			$key = str_replace('"', '', $key);
			if (!$key) continue;
			$q = mysql_query('SELECT FROM `keys` WHERE `key` = "'.$key.'"');
			$r = mysql_fetch_assoc($q);
			if (!$r)
				mysql_query('INSERT INTO `keys` (`key`) VALUES ("'.$key.'")');
		}
		header('Location: /modules/keys.php');
		die();
	}
?>
<!DOCTYPE html>
