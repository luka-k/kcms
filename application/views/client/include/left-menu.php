			...
			<?foreach($left_menu as $item_1):?>
				<li class="accd">
					<? if(!empty($item_1->childs)):?>
						<input type="checkbox" class="parent_checked category-<?=$item_1->id?>"  value="<?=$item_1->id?>" onclick="category_filter('<?=$item_1->id?>')" 
					...
					<? if(!empty($item_1->childs)):?>
						<ul class="show">
							<?foreach ($item_1->childs as $item_2):?>
								<li>
									<input type="checkbox" class="categories_checked" name="cetegories_checked" num="<?=$counter?>" parent="<?=$item_1->id?>" value="<?=$item_2->id?>" id="c_1_1" onclick="category_filter()" 
