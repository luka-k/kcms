<div  class="col_12">
	...
	<select id="lbl_<?=$editors_counter?>" class="menu_items <?=$name?>" name="<?=$name?>" class="col_8">
		<option <?php if ($item_content->$name == 0):?>selected<?php endif; ?> value="0">Родительский пункт</option>
		<?php foreach ($selects[$name] as $select): ?>
			<option <?php if(!empty($select->childs)):?>class="option-1-ch"<?else:?>class="option-1"<?endif;?> value="<?=$select->id?>" <?php if ($item_content->$name == $select->id):?>selected<?php endif; ?><?php if ($item_content->id == $select->id):?>disabled<?php endif; ?>>
				<?=$select->name?>
			</option>
				<?php if(!empty($select->childs)):?>
					<?php foreach ($select->childs as $select_2): ?>
						<option <?php if(!empty($select_2->childs)):?>class="option-2-ch"<?else:?>class="option-2"<?endif;?> value="<?=$select_2->id?>" <?php if ($item_content->$name == $select_2->id):?>selected<?php endif; ?><?php if ($item_content->id == $select_2->id):?>disabled<?php endif; ?>>
							<?=$select_2->name?>
						</option>
