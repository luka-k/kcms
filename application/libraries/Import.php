...
class Import{
	...
	/**
	* »мпортирование издательства hueber
	*
	* 
	*/
		...
		foreach($xml as $item)
		{
			if(!empty($item->productidentifier))
			{
				...
				if(isset($item->language->b252))
				{
					$characteristics[] = array(
						'type' => 'language',
						'value' => (string) $item->language->b252, //€ как понимаю по какому то списку надо по коду название €зыка подставить
	...
	*/
	public function import_csv($filename)
	{
		...
		if ($file) 
		{
			...
			//А тут вызов нужной функции импорта. НАпример:
