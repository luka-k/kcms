...
function validation(element, errorClass){
	...
	input.each(function () {
		...
		factors.forEach(function(item){ 
			...
			if(item == 'unique')
			{
				var data = {value: el.val(), table: $('#table').val(), field: el.prop('name')};
				var json_str = JSON.stringify(data);
				$.post ("/admin/content/unique/", json_str, function(data){
					if(data == false)
					{
						el.attr('placeholder', 'Такой email уже зарегестрирован');
						el.addClass(errorClass);
						isError = true;
					}
				}, "json");
